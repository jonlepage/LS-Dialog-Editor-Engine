...
| `track.ts` | **The traversal, written ONCE, walked in a LOOP.** One `Track` walks the graph; the main flow is one of them, with id 0. Each step RETURNS the next one to `Track.run()` instead of calling it — the recursive walk added three frames per block and a synchronous condition ↔ action loop overflowed the stack (694 passes in TS; a dead process in C# and C++). `run()` is also the ONE error boundary. The traversal used to be written twice — here and in an `AsyncTrack` class — and the two drifted into two shipped bugs. |
| `scene-handle.ts` | `SceneHandleImpl` — the scene: public API, Tier 2 overrides, everything its tracks SHARE (visited set, choice history, pending waits), and closing: `shutdown(reason)`, deadlock detection. It does not walk the graph. |
...
| `validator.ts` | `init()` diagnostics → `DiagnosticReport`. Reads `format`/`version` FIRST. Also reads what the BLOCKS use — calls, arguments, condition tests — against what the export itself declares: always on, warnings only (`UNDECLARED_*`, `UNKNOWN_CHOICE_*`, `EMPTY_FUNCTION`). The `UNKNOWN_FUNCTION`/`UNKNOWN_DICTIONARY*` codes are something else: the header against what the GAME passes in `check`. A diagnostic carries `sceneId` — the STABLE id, it held the path until 2026-09-12 — and `scenePath`. |
...
3. `onBeforeBlock` — fires for every block; the type handler waits for its `resolve()`. `resolve()`
   ...
   A `resolve()` called WHILE `onBeforeBlock` is still running is deferred to its return, like
   `next()` inside a handler — so code written after `resolve()` runs before the block.
...
6. **Any code of the game that throws during the walk** — handler, cleanup, validation,
   `onInvalidateBlock`, `onBeforeBlock`, `onResolveCondition`, `onResolveCharacter`, `onSceneEnter`,
   on ANY track — closes the WHOLE scene (cleanups run, tracks cancelled, `onSceneExit` fired with
   `faulted`) and **then** the error is re-thrown to whoever called `start()`, `next()` or
   `resolve()`. That is problem 11; until 2026-09-12 only the type handler on the main flow was
   actually covered. GDScript has no exceptions: a script error leaves the block waiting.
7. **Every scene ends with a reason**: `onSceneExit` gets `context.reason` — `completed`,
   `cancelled`, `invalidated`, `faulted` or `deadlocked` (then `context.waitingFor` names the
   blocks). `SceneEndReason` is exported. Closing is not re-entrant: a cleanup that cancels its own
   scene does not fire `onSceneExit` twice.
8. **One thread.** C# and C++ remember the thread that started the scene and refuse `next()`,
   `resolve()` and `cancel()` from any other, with an exception that names both — before touching
   anything, so the refused call consumes nothing.
...
A scene where every track left is parked on a `waitForBlocks` closes as **`deadlocked`**. It is
checked when a track ENDS and when one PARKS — checking only the first used to leave a single flow,
parked on a block of a branch it never took, holding the scene open forever.
...
runner; the
TypeScript one is `lsde-ts/src/cross-language-runner.test.ts`. A validation case compares its
diagnostic codes as a whole list, order aside: a runtime that emits a code the spec does not list
fails. Until 2026-09-12 the runners only checked that each listed code was present. `expectedAt`
pins where every diagnostic of a case points: `sceneId`, `scenePath`, `blockId`.

A suite marked `requiresExceptions` throws from a handler; the GDScript runner skips it, saying so.
`lsde-ts/src/traversal-order.test.ts` pins the full callback order of the walk in snapshots — a
change that moves one is a behaviour change, not a refactor.
