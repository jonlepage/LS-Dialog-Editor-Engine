...
Console.WriteLine(
    Dim($"     Stats: {report.Stats.SceneCount} scenes, {report.Stats.BlockCount} blocks, {report.Stats.ConnectionCount} connections"));
...
engine.OnDialog(dialogArgs =>
{
    ...
    var charStr = ch != null
        ? $"{Magenta(ch.Name)} {Dim($"({ch.Emotion ?? "?"})")}"
        : Dim("(no character)");
    var text = block.DialogueText != null && block.DialogueText.ContainsKey(locale)
        ? block.DialogueText[locale]
        : block.Content ?? "—";
    var flags = new List<string>();
    if (block.NativeProperties?.PortPerCharacter == true) flags.Add("portPerCharacter");
    if (block.NativeProperties?.IsAsync == true) flags.Add("async");
    if (block.NativeProperties?.Debug == true) flags.Add("debug");
...
engine.OnChoice(choiceArgs =>
{
    ...
    foreach (var c in ctx.Choices)
    {
        ...
        var choiceText = c.DialogueText != null && c.DialogueText.ContainsKey(locale)
    ...
    Console.WriteLine(Dim($"         -> selecting: {pick.Label ?? pick.Uuid[..Math.Min(8, pick.Uuid.Length)]}"));
...
engine.OnSceneEnter(_ =>
    Console.WriteLine($"\n{Bold(Green("--- Scene Enter ---"))}"));

engine.OnSceneExit(_ =>
    Console.WriteLine($"{Bold(Red("--- Scene Exit ---"))}\n"));

engine.OnValidateNextBlock(valArgs =>
{
    if (valArgs.FromBlock != null)
        Console.WriteLine(Gray($"       [validate] {Label(valArgs.FromBlock)} -> {Label(valArgs.NextBlock)}"));
...
foreach (var uuid in handle.GetVisitedBlocks())
{
    ...
    foreach (var scene in blueprint.Scenes)
    {
        ...
        {
            if (b.Uuid == uuid) { found = b.Label ?? uuid[..Math.Min(8, uuid.Length)]; break; }
        }
        if (found != null) break;
    }
...
Console.WriteLine($"{Bold("Engine:")} running={(!engine.IsRunning() ? Dim("false") : Green("true"))}");
...
class PlaygroundStateBridge : IStateBridge
{
    ...
    {
        Console.WriteLine(_gray($"       [bridge] eval: {condition.Key} {condition.Operator} {condition.Value} -> true"));
        return true;
    }

    public void ExecuteAction(ExportAction action, ActionSignature? signature)
    {
        Console.WriteLine(_gray($"       [bridge] exec: {signature?.Label ?? action.ActionId}({string.Join(", ", action.Params)})"));
    }
