// JSON deserialization implementations for nlohmann/json

#include "json_deserializer.h"

// Helper macros for optional fields
#define OPT(j, field, name) if ((j).contains(name)) (field) = (j).at(name).get<decltype(field)::value_type>()
#define GET(j, field, name) if ((j).contains(name)) (field) = (j).at(name).get<std::decay_t<decltype(field)>>()
