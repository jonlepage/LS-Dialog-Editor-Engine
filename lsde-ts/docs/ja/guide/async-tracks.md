...
`props.waitForBlocks` は、指定された block が**完了する**までその block を保留します。`isAsync` が開く分岐の「合流」側です：ブランチが並列で走り、下流の block はそれが**終わる**まで待ってから再生されます。

**到達ではなく完了です。** リストされた block は、フローがそれを**離れた**時点で数えられます：ゲームが `next()` を呼び、出口ポートが解決され、その block の cleanup が実行された時点です。つまり合流するセリフが dispatch される*前に*吹き出しは画面から消え、音声も止まっています — 合流を描く目的はまさにそれです。

::: warning 変更されたルール
2.x の最初のリリースでは、**到達**だけで十分でした。それはデザイナーが実際に描く形において、このプロパティをほぼ無効にしていました：2 つの block への分岐、そして両方への合流は、登録されたまさにその tick で解除されていたのです — 2 つはほんの数分の 1 ミリ秒前に dispatch されていたため — そして合流するセリフがその上に重なって話していました。

`getVisitedBlocks()` は影響を受けません：プレイヤーが**見た**ものを引き続き列挙し、それには話している途中の block も含まれます。
:::
...
- 1 つではなく、**すべて**が完了している必要があります。
- block が完了すると、それを待っていたものが連鎖的に解放されます。
- 決して完了しない block はトラックを永久に停留させます — プレイヤー入力を永遠に待つ block は決して完了しません。id がシーンの block でない場合、`init()` は `UNKNOWN_WAIT_BLOCK` を報告しますが、実在する block が実際に再生されるかどうかは知り得ません。
