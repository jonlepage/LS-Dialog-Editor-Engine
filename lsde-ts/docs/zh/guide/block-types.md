...
共有 6 种类型：**Dialog**、**Choice**、**Condition**、**Router**、**Action** 和 **Note**。Dialog、Choice、Condition 和 Action 是内容 block，各有专用的 handler（`onDialog`、`onChoice`、`onCondition`、`onAction`）— 四个都是**必需的**，在调用 `start()` 时验证。Router 没有 handler：engine 自己分发它。Note block 会被自动跳过。
...
router block 携带与 condition **相同的 `cases`**，但反过来读。condition 问的是*哪一个*成立，然后从单一 port 出去；router 问的是*哪些*成立：它对**每一个** case 求值，为每个为真的 case 启动其 port，然后总是继续 —— 全部成立走 `then`，任何一个不成立走 `catch`。一个 case 都没有的 router 从 `then` 出去，就像 `Promise.all([])` 会 resolve 一样。

它的 `K*` 路由和任何其他 port 一样被走过：`isAsync` 的目标开启自己的轨道，其余的依次走过，而后续放在**最后**。`catch` 不取消任何东西 —— 为真的 case 的轨道已经在跑了。

**没有 `onRouter` handler**，`start()` 也不要求它：engine 自己分发 router。要观察一个，用 `handle.on_block(id)` / `handle.onBlock(id)`。

<!--@include: ../../_shared/block-router.md-->

完整契约与图示见 [Router block](/zh/guide/router)；router 的路由通常命名的入口端口 `inPortPerCharacter` 见 [分配角色](/zh/guide/character-distribution)。
...
| `type` | `BlockType` | `dialog`、`choice`、`condition`、`router`、`action` 或 `note` |
...
| `cases` | `ConditionCase[]?` | CONDITION 与 ROUTER — 同样的数据，反向读取 |
...
**engine** 从 `props` 中读取的十个属性。id 不会冲突 — LSDE 会拒绝与 native 同名的项目属性 — 所以
区分它们只是一次查找。
...
| `inPortPerCharacter` | `boolean?` | 接线**指名 actor**：link 的 `toPort` 是一个 card id，只有这个 actor 会被提供给 `onResolveCharacter`。见 [分配角色](/zh/guide/character-distribution) |
...
这十个之中，只有**三个**会改变遍历：`isAsync`、`waitForBlocks` 和 `inPortPerCharacter`。其余七个原样交给游戏，由游戏决定
拿它们做什么。
