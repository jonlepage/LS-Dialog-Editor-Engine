...
Il existe 6 types : **Dialog**, **Choice**, **Condition**, **Router**, **Action** et **Note**. Dialog, Choice, Condition et Action sont des blocks de contenu avec un handler dédié (`onDialog`, `onChoice`, `onCondition`, `onAction`) — les quatre sont **required** et validés à l'appel de `start()`. Un Router n'a pas de handler : le engine le dispatche tout seul. Les blocks Note sont automatiquement ignorés.
...
Un block router porte les **mêmes `cases`** qu'une condition et les lit dans l'autre sens. Une condition demande *lequel* tient et sort par un seul port ; un router demande *lesquels* : il évalue **tous** les cas, lance le port de chaque cas vrai, puis continue toujours — par `then` si tous ont tenu, par `catch` si l'un d'eux n'a pas tenu. Un router sans aucun cas sort par `then`, comme `Promise.all([])` se résout.

Ses routes `K*` sont parcourues comme n'importe quel port : une cible `isAsync` ouvre sa propre piste, les autres sont jouées à tour de rôle, et la continuation vient en **dernier**. `catch` n'annule rien — les pistes des cas vrais courent déjà.

Il n'y a **pas de handler `onRouter`** et `start()` n'en exige aucun : le engine dispatche un router tout seul. Pour l'observer, passez par `handle.onBlock(id)`.

<!--@include: ../../_shared/block-router.md-->

Voir [Le bloc Router](/fr/guide/router) pour le contrat complet et les schémas, et [Distribuer les personnages](/fr/guide/character-distribution) pour `inPortPerCharacter` — le port d'entrée que les routes d'un router nomment typiquement.
...
| `type` | `BlockType` | `dialog`, `choice`, `condition`, `router`, `action` ou `note` |
...
| `cases` | `ConditionCase[]?` | CONDITION et ROUTER — les mêmes données, lues dans les deux sens |
...
Les dix propriétés que le **engine** lit, prises dans `props`. Les ids ne peuvent pas entrer en collision — LSDE refuse une propriété de projet portant un nom natif — donc les distinguer est une simple recherche.
...
| `inPortPerCharacter` | `boolean?` | Le fil **nomme l'acteur** : le `toPort` d'un lien est un id de card, et seul cet acteur est offert à `onResolveCharacter`. Voir [Distribuer les personnages](/fr/guide/character-distribution) |
...
Seules **trois** de ces propriétés changent quelque chose au parcours : `isAsync`, `waitForBlocks` et `inPortPerCharacter`. Les sept autres sont transmises intactes au jeu, qui décide de ce qu'il en fait.
