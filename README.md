...
| ------------------------------- | ---------- | ------------------------ | ------- |
| [lsde-ts](lsde-ts/)             | TypeScript | Reference implementation | 485/485 |
| [lsde-csharp](lsde-csharp/)     | C#         | Unity, .NET              | 227/227 |
| [lsde-cpp](lsde-cpp/)           | C++        | Unreal, custom engines   | 129/129 |
| [lsde-gdscript](lsde-gdscript/) | GDScript   | Godot 4                  | 358/358 |
...
- **Native properties are data** — `delay`, `timeout`, `debug` etc. are handed to your handler untouched; you decide what they mean. Three exceptions the engine does act on: `isAsync` spawns a parallel track, `waitForBlocks` parks one, `inPortPerCharacter` lets the wire name the actor
...
output, run by all four — 81 cases in 72 suites, generated by `tests/generate-specs.py`.
